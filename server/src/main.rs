...
use dashmap::DashMap;
use l_lang::generated::Parser;
use l_lang::parser::{tokenize, Token};
use l_language_server::chumsky::{
    parse, type_inference, Func, ImCompleteSemanticToken, ParserResult,
};
use l_language_server::jump_definition::get_definition;
use l_language_server::reference::get_reference;
use l_language_server::semantic_token::{semantic_token_from_ast, LEGEND_TYPE};
use l_language_server::CstWithSource;
use log::debug;
use ropey::Rope;
...
struct Backend {
    client: Client,
    ast_map: DashMap<String, CstWithSource>,
...
impl LanguageServer for Backend {
    ...
    async fn inlay_hint(
        ...
        //             k.end,
        //             match v {
        //                 l_language_server::chumsky::Value::Null => "null".to_string(),
        //                 l_language_server::chumsky::Value::Bool(_) => "bool".to_string(),
        //                 l_language_server::chumsky::Value::Num(_) => "number".to_string(),
        //                 l_language_server::chumsky::Value::Str(_) => "string".to_string(),
        //                 l_language_server::chumsky::Value::List(_) => "[]".to_string(),
        //                 l_language_server::chumsky::Value::Func(_) => v.to_string(),
        //             },
        //         )
        //     })
        //     .filter_map(|item| {
...
impl Backend {
    async fn on_change(&self, params: TextDocumentItem) {
        ...
        let (cst_with_source, diags) = CstWithSource::new(params.text);
        let mut diagnostics = vec![];
        for d in diags {
            d.labels.into_iter().for_each(|label| {
                let start = label.range.start;
                let end = label.range.end;
                let start_pos =
                    offset_to_position(start, &rope).expect("should have a start position");
                let end_pos = offset_to_position(end, &rope).expect("should have an end position");
...
fn offset_to_position(offset: usize, rope: &Rope) -> Option<Position> {
    ...
    let offset_of_line = rope.try_line_to_char(line).ok()?;
    let column_u16 = rope.get_slice(offset_of_line..offset)?.len_utf16_cu();
    Some(Position::new(line as u32, column_u16 as u32))
}
